...
### 2026-07-24 变更：首页 3D 地球只上真实活跃用户，今日活跃优先

**需求**：大球剔除注册后零行为的僵尸号；展示规则为「今日活跃优先 + 最近活跃补齐」，球永远不空。

**代码改动**：`src/services/home_store.rs::online_globe`
- 由 LEFT JOIN 改为 INNER JOIN 活动汇总表：只展示有过真实行为（学习/成语/打卡/每日任务/朗诵/动态/评论）的用户，零行为账号不再上球。
- 排序：`(last_active_at 的北京时间日期 = 今天) DESC, last_active_at DESC`——今日活跃排最前，其余按最近活跃补齐，上限 100。
- count 字段不变，仍是累计注册用户数。

**发布方式**：标准流程（第 5 章），SSH 目标 `ubuntu@192.144.133.222`。无数据迁移。

**验证**：`curl https://www.duwei.cloud/api/home/online-globe` 实返 users=67（有行为用户总数），较上一版 85 剔除 18 个零行为账号。
